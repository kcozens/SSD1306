    ...
    while (done == 0) {
        ...
        for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
            ...
            if ((strncmp("wlx", ifa->ifa_name, 3 ) == 0) && family == AF_INET) {
                ...
                printf("%-8s <%s>\n", ifa->ifa_name, host);
                snprintf(text_buffer, sizeof(text_buffer), "%.3s: %s", ifa->ifa_name, host);
                ssd1306DrawString(0, row * 8, text_buffer, 1, WHITE, LAYER0);
                row++;
            }
        }
        ...
        /* Memory usage */
        float m = GetMemUsage();
        snprintf(text_buffer, sizeof(text_buffer), "Mem used: %4.1f%%", m*100);
        printf("%s\n", text_buffer);
        ssd1306DrawString(4, 2, text_buffer, 1, WHITE, LAYER0);
        ssd1306DrawRect(0, 0, 127, 13, INVERSE, LAYER0);
        /* Invert mem usage text area based on % of memory used */
        ssd1306FillRect(2, 2, (int)(124 * m), 9, INVERSE, LAYER0);
